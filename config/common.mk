...
# Terminal Emulator
PRODUCT_COPY_FILES +=  \
    vendor/cm/prebuilt/common/apk/Term.apk:system/app/Term/Term.apk \
    vendor/cm/prebuilt/common/lib/libjackpal-androidterm4.so:system/app/Term/lib/arm/libjackpal-androidterm4.so
...
ifeq ($(CM_BUILDTYPE), RELEASE)
    ...
    ifeq ($(PRODUCT_VERSION_MINOR),0)
        CM_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-$(shell date +%Y%m%d)-$(CM_BUILD)
		           MOD_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-$(shell date +%Y%m%d)-$(CM_EXTRAVERSION)-$(CM_BUILD)
    else
        CM_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-$(shell date +%Y%m%d)-$(CM_BUILD)
		           MOD_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-$(shell date +%Y%m%d)-$(CM_EXTRAVERSION)-$(CM_BUILD)
    endif
...
#lets kill goo.im as its a liar
PRODUCT_PROPERTY_OVERRIDES += \
  ro.goo.developerid=nullDev
